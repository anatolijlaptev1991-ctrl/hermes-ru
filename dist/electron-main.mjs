...
function pathWithGlobalRemoteProfile(path14, profile, opts = {}) {
  const scopedProfile = connectionScopeKey(profile);
  if (!scopedProfile || !opts.globalRemote || opts.profileRemoteOverride) {
    return path14;
  }
  const rawPath = String(path14 || "");
  if (!rawPath) {
    return path14;
  }
  let parsed;
...
function resolveRemovableAppPath(execPath, platform, env2 = {}) {
  ...
  const p2 = platform === "win32" ? path3.win32 : path3.posix;
  if (platform === "darwin") {
...
var DATA_URL_READ_MAX_BYTES = 16 * 1024 * 1024;
...
function resolveRequestedPathForIpc(filePath, options = {}) {
  ...
  if (raw === "~" || raw.startsWith("~/") || raw.startsWith("~\\")) {
    raw = path4.join(os.homedir(), raw.slice(1));
  }
  if (/^file:/i.test(raw)) {
    let resolvedPath2;
    try {
    ...
    return path4.resolve(resolvedPath2);
  }
  ...
  const resolvedBase = path4.resolve(safeBaseInput);
  rejectUnsafePathSyntax(resolvedBase, purpose);
  const resolvedPath = path4.resolve(resolvedBase, raw);
...
async function resolveReadableFileForIpc(filePath, options = {}) {
  ...
  try {
    await fsImpl.promises.access(resolvedPath, fs4.constants.R_OK);
  } catch {
...
async function entryForDirent(dirent, resolved, fsImpl) {
  const fullPath = path5.join(resolved, dirent.name);
...
async function scanGitRepos(roots, options = {}) {
  ...
  async function walk(dir, depth) {
    ...
    if (entries.some((entry) => entry.name === ".git" && entry.isDirectory())) {
      ...
      found.set(root, path6.basename(root) || root);
      return;
    }
    const subdirs = [];
    for (const entry of entries) {
      ...
      subdirs.push(path6.join(dir, entry.name));
    }
...
function ghEnv(ghBin) {
  const extra = [ghBin ? path7.dirname(ghBin) : "", "/opt/homebrew/bin", "/usr/local/bin", "/usr/bin"].filter(
    (dir) => dir && dir !== "."
  );
  return { ...process.env, PATH: [...extra, process.env.PATH].filter(Boolean).join(path7.delimiter) };
}
...
function gitFor(cwd, gitBin) {
  return esm_default({ baseDir: cwd, binary: gitBin || "git", maxConcurrentProcesses: 4, trimmed: false });
}
function resolveRenamePath(raw) {
  const path14 = String(raw || "").trim();
  if (!path14.includes(" => ")) {
    return path14;
  }
  const brace = path14.match(/^(.*)\{(.*) => (.*)\}(.*)$/);
  if (brace) {
    const [, prefix, , to, suffix] = brace;
    return `${prefix}${to}${suffix}`.replace(/\/{2,}/g, "/");
  }
  return path14.split(" => ").pop().trim();
}
  ...
  try {
    if (scope === "branch" || scope === "lastTurn") {
      ...
      if (scope === "lastTurn") {
        ...
        for (const path14 of status2.not_added) {
          if (!files2.some((f) => f.path === path14)) {
            files2.push({ path: path14, added: 0, removed: 0, status: "?", staged: false });
          }
        }
      }
  ...
  try {
    ...
    for (let i2 = 0; i2 < untracked.length; i2 += UNTRACKED_LINE_COUNT_CONCURRENCY) {
      const batch = await Promise.all(
        untracked.slice(i2, i2 + UNTRACKED_LINE_COUNT_CONCURRENCY).map((path14) => untrackedInsertions(cwd, path14))
      );
...
function findGitRoot(start, fsImpl = fs8) {
  let dir = start;
  for (let i2 = 0; i2 < 50; i2 += 1) {
    try {
      if (fsImpl.existsSync(path8.join(dir, ".git"))) {
        return dir;
      }
    } catch {
      return null;
...
async function gitRootForIpc(startPath, options = {}) {
  ...
  try {
    ...
    const start = stat.isDirectory() ? resolved : path8.dirname(resolved);
...
async function addExistingBranchWorktree(gitBin, root, name) {
  ...
  const dir = uniqueDir(path9.join(root, ".worktrees", slugify(branch)));
...
async function addWorktree(repoPath, options, gitBin) {
  ...
  const dir = uniqueDir(path9.join(root, ".worktrees", slug));
...
function createLinkTitleWindow(BrowserWindow2, partitionSession) {
...
function markerPath(hermesHome) {
  return path10.join(hermesHome, ".hermes-update-in-progress");
}
...
function resolveUnpackedRelease(execPath, updateRoot, platform) {
  ...
  const releaseDir = path11.join(updateRoot, "apps", "desktop", "release");
  const unpacked = path11.join(releaseDir, unpackedDirName(platform));
  const normalizedExec = path11.resolve(String(execPath));
  const withSep = unpacked.endsWith(path11.sep) ? unpacked : unpacked + path11.sep;
...
function sandboxPreflight(unpackedDir, statSync) {
  ...
  const sandboxPath = path11.join(unpackedDir, "chrome-sandbox");
  let st;
  try {
...
function isPackagedInstallPath(dir, { installRoots, isPackaged }) {
  ...
  let resolved;
  try {
    resolved = path12.resolve(String(dir));
  } catch {
    return false;
  }
  const roots = new Set((installRoots ?? []).filter(Boolean).map((candidate) => path12.resolve(String(candidate))));
...
var ZOOM_REASSERT_WINDOW_EVENTS = ["show", "restore"];
...
if (USER_DATA_OVERRIDE) {
  const resolvedUserData = path13.resolve(USER_DATA_OVERRIDE);
  fs11.mkdirSync(resolvedUserData, { recursive: true });
...
var DARWIN_MAJOR = IS_MAC ? Number.parseInt(os4.release(), 10) || 0 : 0;
var APP_ROOT = app.getAppPath();
var PRELOAD_PATH = path13.join(APP_ROOT, "dist", "electron-preload.js");
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs11.existsSync("/dev/dxg")) {
...
var SOURCE_REPO_ROOT = path13.resolve(APP_ROOT, "../..");
var INSTALL_STAMP_SCHEMA_VERSION = 1;
function loadInstallStamp() {
  const candidates = [
    process.resourcesPath ? path13.join(process.resourcesPath, "install-stamp.json") : null,
    path13.join(APP_ROOT, "build", "install-stamp.json")
  ].filter(Boolean);
  for (const p2 of candidates) {
    try {
      const raw = fs11.readFileSync(p2, "utf8");
...
function resolveHermesHome() {
  ...
  if (USER_DATA_OVERRIDE) {
    return path13.join(path13.resolve(USER_DATA_OVERRIDE), "hermes-home");
  }
  if (IS_WINDOWS2) {
  ...
  if (IS_WINDOWS2 && process.env.LOCALAPPDATA) {
    const localappdata = path13.join(process.env.LOCALAPPDATA, "hermes");
    const legacy = path13.join(app.getPath("home"), ".hermes");
    if (!directoryExists(localappdata) && directoryExists(legacy)) {
      return legacy;
    }
    return localappdata;
  }
  return path13.join(app.getPath("home"), ".hermes");
}
var HERMES_HOME = resolveHermesHome();
function hermesManagedNodePathEntries() {
  const root = path13.join(HERMES_HOME, "node");
  const bin = path13.join(root, "bin");
  const entries = IS_WINDOWS2 ? [root, bin] : [bin, root];
  return entries.filter(directoryExists);
}
function pathWithHermesManagedNode(...entries) {
  return [...hermesManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path13.delimiter);
}
var ACTIVE_HERMES_ROOT = path13.join(HERMES_HOME, "hermes-agent");
var VENV_ROOT = path13.join(ACTIVE_HERMES_ROOT, "venv");
var BOOTSTRAP_COMPLETE_MARKER = path13.join(ACTIVE_HERMES_ROOT, ".hermes-bootstrap-complete");
var BOOTSTRAP_MARKER_SCHEMA_VERSION = 1;
var DESKTOP_CONNECTION_CONFIG_PATH = path13.join(app.getPath("userData"), "connection.json");
var DESKTOP_UPDATE_CONFIG_PATH = path13.join(app.getPath("userData"), "updates.json");
var DESKTOP_WINDOW_STATE_PATH = path13.join(app.getPath("userData"), "window-state.json");
var DESKTOP_PROFILE_CONFIG_PATH = path13.join(app.getPath("userData"), "active-profile.json");
var PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/;
var DEFAULT_UPDATE_BRANCH = "main";
var DESKTOP_LOG_PATH = path13.join(HERMES_HOME, "logs", "desktop.log");
...
var APP_ICON_PATHS = [
  path13.join(APP_ROOT, "public", "apple-touch-icon.png"),
  path13.join(APP_ROOT, "dist", "apple-touch-icon.png"),
  path13.join(unpackedPathFor(APP_ROOT), "dist", "apple-touch-icon.png")
];
var rendererTitleBarTheme = null;
var terminalSessions = /* @__PURE__ */ new Map();
var NATIVE_THEME_CONFIG_PATH = path13.join(app.getPath("userData"), "native-theme.json");
var THEME_SOURCES = /* @__PURE__ */ new Set(["dark", "light", "system"]);
function readPersistedThemeSource() {
  try {
    const parsed = JSON.parse(fs11.readFileSync(NATIVE_THEME_CONFIG_PATH, "utf8"));
...
function writePersistedThemeSource(mode) {
  try {
    fs11.mkdirSync(path13.dirname(NATIVE_THEME_CONFIG_PATH), { recursive: true });
    fs11.writeFileSync(NATIVE_THEME_CONFIG_PATH, JSON.stringify({ themeSource: mode }, null, 2), "utf8");
  } catch (error) {
    rememberLog(`[theme] write native theme failed: ${error.message}`);
  }
}
nativeTheme.themeSource = readPersistedThemeSource();
var TRANSLUCENCY_CONFIG_PATH = path13.join(app.getPath("userData"), "translucency.json");
...
function readPersistedTranslucency() {
  try {
    return clampIntensity(JSON.parse(fs11.readFileSync(TRANSLUCENCY_CONFIG_PATH, "utf8")).intensity);
  } catch {
    return 0;
  }
}
function writePersistedTranslucency(intensity) {
  try {
    fs11.mkdirSync(path13.dirname(TRANSLUCENCY_CONFIG_PATH), { recursive: true });
    fs11.writeFileSync(TRANSLUCENCY_CONFIG_PATH, JSON.stringify({ intensity }, null, 2), "utf8");
  } catch (error) {
  ...
  try {
    ...
    if (!mimeType.startsWith("image/")) {
      ...
      try {
        ...
        const bytesRead = fs11.readSync(fd, sample, 0, sample.length, 0);
...
function registerMediaProtocol() {
  protocol.handle(MEDIA_PROTOCOL, async (request2) => {
    ...
    if (!STREAMABLE_MEDIA_EXTS.has(path13.extname(resolvedPath).toLowerCase())) {
      return new Response("Unsupported media type", { status: 415 });
    }
    return electronNet.fetch(pathToFileURL2(resolvedPath).toString(), {
...
function rotateDesktopLogIfNeededSync() {
  let size;
  try {
    size = fs11.statSync(DESKTOP_LOG_PATH).size;
  } catch {
    return;
  }
...
async function rotateDesktopLogIfNeededAsync() {
  let size;
  try {
    size = (await fs11.promises.stat(DESKTOP_LOG_PATH)).size;
  } catch {
    return;
  }
  for (const [op, src, dst] of planDesktopLogRotation(size)) {
    try {
      if (op === "rm") {
        await fs11.promises.rm(src, { force: true });
      } else {
        await fs11.promises.rename(src, dst);
      }
    } catch {
    }
  }
...
function flushDesktopLogBufferSync() {
  ...
  desktopLogBuffer = "";
  try {
    fs11.mkdirSync(path13.dirname(DESKTOP_LOG_PATH), { recursive: true });
    rotateDesktopLogIfNeededSync();
    fs11.appendFileSync(DESKTOP_LOG_PATH, chunk);
  } catch {
  }
}
function flushDesktopLogBufferAsync() {
  ...
  desktopLogFlushPromise = desktopLogFlushPromise.then(async () => {
    await fs11.promises.mkdir(path13.dirname(DESKTOP_LOG_PATH), { recursive: true });
    await rotateDesktopLogIfNeededAsync();
    await fs11.promises.appendFile(DESKTOP_LOG_PATH, chunk);
  }).catch(() => {
  });
  return desktopLogFlushPromise;
}
...
function openExternalUrl(rawUrl) {
  ...
  if (IS_WSL) {
    ...
    const proc = spawn3("cmd.exe", ["/c", "start", '""', url], {
      detached: true,
      stdio: "ignore",
      windowsHide: true
    });
...
async function openPreviewInBrowser(rawUrl) {
  ...
  if (parsed.protocol === "file:") {
    ...
    await shell.openExternal(pathToFileURL2(localPath).toString());
    return true;
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    const confDir = path13.join(app.getPath("home"), ".config", "fontconfig", "conf.d");
    const confPath = path13.join(confDir, "99-hermes-wsl-windows-fonts.conf");
    let existing = "";
    try {
      existing = fs11.readFileSync(confPath, "utf8");
    } catch {
      existing = "";
    }
    ...
    fs11.mkdirSync(confDir, { recursive: true });
    fs11.writeFileSync(
      confPath,
      `<?xml version="1.0"?>
...
`
    ...
    const cache = spawn3("fc-cache", ["-f", fontsDir], { detached: true, stdio: "ignore" });
...
async function waitForUpdateToFinish() {
  ...
  while (marker && Date.now() < deadline) {
    ...
    await new Promise((r2) => setTimeout(r2, UPDATE_WAIT_POLL_MS));
...
function findOnPath(command) {
  ...
  if (path13.isAbsolute(command) || command.includes(path13.sep) || IS_WINDOWS2 && command.includes("/")) {
  ...
  const extensions = IS_WINDOWS2 ? [...(process.env.PATHEXT || ".COM;.EXE;.BAT;.CMD").split(";").filter(Boolean), ""] : [""];
  for (const entry of pathEntries) {
    for (const extension of extensions) {
      const candidate = path13.join(entry, `${command}${extension}`);
...
function isCommandScript(command) {
  return IS_WINDOWS2 && /\.(cmd|bat)$/i.test(command || "");
}
function unwrapWindowsVenvHermesCommand(command, backendArgs) {
  if (!IS_WINDOWS2 || !command || isCommandScript(command)) {
    return null;
  }
  const resolved = path13.resolve(String(command));
  if (!/^hermes(?:\.exe)?$/i.test(path13.basename(resolved))) {
    return null;
  }
  const scriptsDir = path13.dirname(resolved);
  if (path13.basename(scriptsDir).toLowerCase() !== "scripts") {
    return null;
  }
  const venvRoot = path13.dirname(scriptsDir);
  const python = getVenvPython(venvRoot);
  if (!fileExists(python)) {
    return null;
  }
  const root = path13.dirname(venvRoot);
  if (!canImportHermesCli(python, {
    env: {
      PYTHONPATH: [...directoryExists(root) ? [root] : [], process.env.PYTHONPATH].filter(Boolean).join(path13.delimiter)
    }
  })) {
    rememberLog(
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs11.readFileSync(path13.join(backend.root, "hermes_cli", "subcommands", "dashboard.py"), "utf8");
  ...
  if (supported === null) {
    try {
      ...
      execFileSync4(backend.command, [...prefix, "serve", "--help"], {
...
function normalizeExecutablePathForCompare(commandPath) {
  ...
  let resolved = path13.resolve(String(commandPath));
  try {
    resolved = fs11.realpathSync.native ? fs11.realpathSync.native(resolved) : fs11.realpathSync(resolved);
  } catch {
  }
  return IS_WINDOWS2 ? resolved.toLowerCase() : resolved;
}
function looksLikeDesktopAppBinary(commandPath) {
  if (!IS_WINDOWS2 || !commandPath) {
    return false;
  }
  ...
  const resourcesDir = path13.join(path13.dirname(resolved), "resources");
  return fileExists(path13.join(resourcesDir, "app.asar")) || directoryExists(path13.join(resourcesDir, "app.asar.unpacked"));
}
function isHermesSourceRoot(root) {
  return directoryExists(root) && fileExists(path13.join(root, "hermes_cli", "main.py"));
}
function findPythonForRoot(root) {
  ...
  const relativePaths = IS_WINDOWS2 ? [path13.join(".venv", "Scripts", "python.exe"), path13.join("venv", "Scripts", "python.exe")] : [path13.join(".venv", "bin", "python"), path13.join("venv", "bin", "python")];
  for (const relativePath of relativePaths) {
    const candidate = path13.join(root, relativePath);
    if (fileExists(candidate)) {
      return candidate;
    }
  }
...
function findSystemPython() {
    ...
    for (const version of SUPPORTED_VERSIONS) {
      try {
        const out = execFileSync4(
          ...
          hiddenWindowsChildOptions2({ encoding: "utf8", stdio: ["ignore", "pipe", "ignore"] })
        );
        const match = out.match(/REG_SZ\s+(.+?)\s*$/m);
        if (match) {
          const installPath = match[1].trim();
          const pythonExe = path13.join(installPath, "python.exe");
  ...
  for (const versionDir of SUPPORTED_VERSIONS_NO_DOT) {
    const systemWide = path13.join(programFiles, `Python${versionDir}`, "python.exe");
    if (fileExists(systemWide)) {
      return systemWide;
    }
    if (localAppData) {
      const perUser = path13.join(localAppData, "Programs", "Python", `Python${versionDir}`, "python.exe");
...
function findGitBash() {
  ...
  const candidates = [];
  if (localAppData) {
    candidates.push(path13.join(localAppData, "hermes", "git", "bin", "bash.exe"));
    candidates.push(path13.join(localAppData, "hermes", "git", "usr", "bin", "bash.exe"));
  }
  candidates.push(path13.join(process.env["ProgramFiles"] || "C:\\Program Files", "Git", "bin", "bash.exe"));
  candidates.push(path13.join(process.env["ProgramFiles(x86)"] || "C:\\Program Files (x86)", "Git", "bin", "bash.exe"));
  if (localAppData) {
    candidates.push(path13.join(localAppData, "Programs", "Git", "bin", "bash.exe"));
  }
...
function getVenvPython(venvRoot) {
  return path13.join(venvRoot, IS_WINDOWS2 ? path13.join("Scripts", "python.exe") : path13.join("bin", "python"));
}
function getVenvSitePackagesEntries(venvRoot) {
  ...
  if (IS_WINDOWS2) {
    const sitePackages = path13.join(venvRoot, "Lib", "site-packages");
  ...
  const version = (() => {
    try {
      const cfg = fs11.readFileSync(path13.join(venvRoot, "pyvenv.cfg"), "utf8");
  ...
  if (version) {
    const sitePackages = path13.join(venvRoot, "lib", `python${version}`, "site-packages");
    if (directoryExists(sitePackages)) {
      entries.push(sitePackages);
    }
  }
...
function makeDashboardReadyFile() {
  const dir = path13.join(app.getPath("userData"), "backend-ready");
  fs11.mkdirSync(dir, { recursive: true });
  return path13.join(dir, `dashboard-${process.pid}-${Date.now()}-${crypto.randomBytes(6).toString("hex")}.json`);
}
...
function resolveGitBinary() {
  ...
  const candidates = [];
  if (localAppData) {
    candidates.push(path13.join(localAppData, "hermes", "git", "cmd", "git.exe"));
    candidates.push(path13.join(localAppData, "hermes", "git", "bin", "git.exe"));
  }
  candidates.push(path13.join(process.env["ProgramFiles"] || "C:\\Program Files", "Git", "cmd", "git.exe"));
  candidates.push(path13.join(process.env["ProgramFiles(x86)"] || "C:\\Program Files (x86)", "Git", "cmd", "git.exe"));
  if (localAppData) {
    candidates.push(path13.join(localAppData, "Programs", "Git", "cmd", "git.exe"));
  }
...
function resolveGhBinary() {
  ...
  const candidates = [];
  if (IS_WINDOWS2) {
    candidates.push(path13.join(process.env["ProgramFiles"] || "C:\\Program Files", "GitHub CLI", "gh.exe"));
    if (process.env.LOCALAPPDATA) {
      candidates.push(path13.join(process.env.LOCALAPPDATA, "Microsoft", "WinGet", "Links", "gh.exe"));
    }
  } else {
    const home = app.getPath("home");
    candidates.push("/opt/homebrew/bin/gh", "/usr/local/bin/gh", "/usr/bin/gh", path13.join(home, ".local", "bin", "gh"));
  }
...
function readDesktopUpdateConfig() {
  try {
    const parsed = JSON.parse(fs11.readFileSync(DESKTOP_UPDATE_CONFIG_PATH, "utf8"));
...
function writeFileAtomic(targetPath, data, encoding) {
  ...
  fs11.writeFileSync(tmp, data, encoding);
  fs11.renameSync(tmp, targetPath);
}
function writeDesktopUpdateConfig(config) {
  fs11.mkdirSync(path13.dirname(DESKTOP_UPDATE_CONFIG_PATH), { recursive: true });
  writeFileAtomic(DESKTOP_UPDATE_CONFIG_PATH, JSON.stringify(config, null, 2));
}
function readWindowState() {
  try {
    return sanitizeWindowState(JSON.parse(fs11.readFileSync(DESKTOP_WINDOW_STATE_PATH, "utf8")));
  } catch {
    return null;
  }
}
function persistWindowState() {
  ...
  try {
    ...
    fs11.mkdirSync(path13.dirname(DESKTOP_WINDOW_STATE_PATH), { recursive: true });
    writeFileAtomic(
...
function resolveUpdateRoot() {
  const candidates = [
    process.env.HERMES_DESKTOP_HERMES_ROOT && path13.resolve(process.env.HERMES_DESKTOP_HERMES_ROOT),
  ...
  return candidates.find((c3) => directoryExists(path13.join(c3, ".git"))) || candidates[0] || ACTIVE_HERMES_ROOT;
}
function runGit2(args, options = {}) {
  return new Promise((resolve, reject) => {
    const child = spawn3(
      resolveGitBinary(),
      IS_WINDOWS2 ? ["-c", "windows.appendAtomically=false", ...args] : args,
      hiddenWindowsChildOptions2({
        cwd: options.cwd,
...
async function checkUpdates() {
  ...
  const gitDir = path13.join(updateRoot, ".git");
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS2 ? "hermes-setup.exe" : "hermes-setup";
  const candidate = path13.join(HERMES_HOME, name);
...
function repairMacUpdaterHelper(updater) {
  ...
  try {
    execFileSync4("/usr/bin/xattr", ["-cr", updater], { stdio: "ignore" });
  } catch (err) {
    rememberLog(`[updates] macOS updater helper quarantine repair skipped: ${err.message}`);
  }
  try {
    execFileSync4("/usr/bin/codesign", ["--verify", updater], { stdio: "ignore" });
    return;
  } catch {
  }
  try {
    execFileSync4("/usr/bin/codesign", ["--force", "--sign", "-", updater], { stdio: "ignore" });
...
function venvHermesShimPath(updateRoot) {
  return IS_WINDOWS2 ? path13.join(updateRoot, "venv", "Scripts", "hermes.exe") : path13.join(updateRoot, "venv", "bin", "hermes");
}
...
function forceKillProcessTree(pid) {
  ...
  try {
    execFileSync4("taskkill", ["/PID", String(pid), "/T", "/F"], hiddenWindowsChildOptions2({ stdio: "ignore" }));
  } catch {
  }
}
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    const venvBin = path13.join(updateRoot, "venv", IS_WINDOWS2 ? "Scripts" : "bin");
    ...
    const child = spawn3(updater, updaterArgs, {
      cwd: HERMES_HOME,
      env: {
        ...process.env,
        HERMES_HOME,
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const updateRoot = resolveUpdateRoot();
  const { branch: configuredBranch } = readDesktopUpdateConfig();
  const branch = directoryExists(path13.join(updateRoot, ".git")) ? await resolveHealedBranch(updateRoot, configuredBranch || DEFAULT_UPDATE_BRANCH) : configuredBranch || DEFAULT_UPDATE_BRANCH;
  const venvBin = path13.join(updateRoot, "venv", IS_WINDOWS2 ? "Scripts" : "bin");
  const venvHermes = path13.join(venvBin, IS_WINDOWS2 ? "hermes.exe" : "hermes");
  const venvPython = path13.join(venvBin, IS_WINDOWS2 ? "python.exe" : "python");
  const haveRealInstall = fileExists(venvPython) || fileExists(venvHermes) || fileExists(path13.join(updateRoot, ".hermes-bootstrap-complete"));
...
function resolveHermesCliBinary(updateRoot) {
  const venvHermes = path13.join(updateRoot, "venv", "bin", "hermes");
...
function runningAppBundle() {
  ...
  let dir = path13.dirname(app.getPath("exe"));
...
async function applyUpdatesPosixInApp(opts) {
  ...
  const env2 = {
    HERMES_HOME,
    PATH: pathWithHermesManagedNode(path13.join(updateRoot, "venv", "bin"))
  };
  ...
  if (!IS_MAC) {
    ...
    const preflight = underUnpacked ? sandboxPreflight(unpackedDir, (p2) => fs11.statSync(p2)) : { ok: false, reason: "not-under-unpacked", path: null };
    ...
    if (outcome === "relaunch") {
      ...
      const scriptPath2 = path13.join(app.getPath("temp"), `hermes-desktop-update-${Date.now()}.sh`);
      try {
        fs11.writeFileSync(scriptPath2, relaunchScript, { mode: 493 });
        const child2 = spawn3("/bin/bash", [scriptPath2], { detached: true, stdio: "ignore" });
        child2.unref();
        rememberLog(
  ...
  const rebuiltApp = [
    path13.join(updateRoot, "apps", "desktop", "release", "mac-arm64", "Hermes.app"),
    path13.join(updateRoot, "apps", "desktop", "release", "mac", "Hermes.app")
...
/usr/bin/open "$DST"
`;
  const scriptPath = path13.join(app.getPath("temp"), `hermes-desktop-update-${Date.now()}.sh`);
  try {
    fs11.writeFileSync(scriptPath, swapScript, { mode: 493 });
  } catch (err) {
    emitUpdateProgress({
      stage: "done",
  ...
  const child = spawn3("/bin/bash", [scriptPath], { detached: true, stdio: "ignore" });
  child.unref();
...
function isActiveRuntimeUsable() {
  ...
  return isHermesSourceRoot(ACTIVE_HERMES_ROOT) && fileExists(venvPython) && canImportHermesCli(venvPython, {
    env: {
      PYTHONPATH: [ACTIVE_HERMES_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path13.delimiter)
    }
  });
}
...
function writeBootstrapMarker(payload) {
  fs11.mkdirSync(path13.dirname(BOOTSTRAP_COMPLETE_MARKER), { recursive: true });
  const merged = {
...
function resolveWebDist() {
  ...
  if (override && directoryExists(path13.resolve(override))) {
    return path13.resolve(override);
  }
  const unpackedDist = path13.join(unpackedPathFor(APP_ROOT), "dist");
  if (directoryExists(unpackedDist)) {
    return unpackedDist;
  }
  const fallback = path13.join(APP_ROOT, "dist");
...
function resolveRendererIndex() {
  const candidates = [path13.join(APP_ROOT, "dist", "index.html"), path13.join(resolveWebDist(), "index.html")];
...
function isPackagedInstallPath2(dir) {
  return isPackagedInstallPath(dir, {
    ...
    installRoots: [
      APP_ROOT,
      path13.dirname(process.execPath),
...
function resolveHermesCwd() {
  ...
  for (const candidate of candidates) {
    ...
    const resolved = path13.resolve(String(candidate));
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  try {
    const resolved = path13.resolve(trimmed2);
    if (directoryExists(resolved)) {
...
function defaultProjectDirConfigPath() {
  return path13.join(app.getPath("userData"), DEFAULT_PROJECT_DIR_CONFIG_FILENAME);
}
function readDefaultProjectDir() {
  try {
    const raw = fs11.readFileSync(defaultProjectDirConfigPath(), "utf8");
    const parsed = JSON.parse(raw);
    if (parsed && typeof parsed.dir === "string" && parsed.dir.trim()) {
      const resolved = path13.resolve(parsed.dir);
      if (directoryExists(resolved)) {
        return resolved;
      }
    }
...
function writeDefaultProjectDir(dir) {
  ...
  const payload = dir ? JSON.stringify({ dir: path13.resolve(dir) }, null, 2) : JSON.stringify({}, null, 2);
  try {
    fs11.mkdirSync(path13.dirname(target), { recursive: true });
    fs11.writeFileSync(target, payload, "utf8");
  } catch (error) {
...
function createPythonBackend(root, label, backendArgs, options = {}) {
  ...
  const venvRoot = path13.join(root, "venv");
  const venvPython = getVenvPython(venvRoot);
  const command = IS_WINDOWS2 && fileExists(venvPython) ? venvPython : python;
  return {
    kind: "python",
    label,
    command,
...
function resolveHermesBackend(backendArgs) {
  const overrideRoot = process.env.HERMES_DESKTOP_HERMES_ROOT && path13.resolve(process.env.HERMES_DESKTOP_HERMES_ROOT);
  ...
  if (process.env.HERMES_DESKTOP_IGNORE_EXISTING !== "1") {
    ...
    if (hermesCommand) {
      ...
      if (verifyHermesCli(hermesCommand, { shell: shellForProbe })) {
        return unwrapWindowsVenvHermesCommand(hermesCommand, backendArgs) || {
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === "bootstrap-needed") {
    ...
    const bootstrapResult = await runBootstrap({
      ...
      hermesHome: HERMES_HOME,
      logRoot: path13.join(HERMES_HOME, "logs"),
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Hermes bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ""}: ${bootstrapResult.error || "unknown error"}. Check ${path13.join(HERMES_HOME, "logs", "desktop.log")} for the full transcript.`
      );
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const client = parsed.protocol === "https:" ? https3 : http;
...
function filenameFromUrl(rawUrl, fallback = "image") {
  try {
    ...
    const base = path13.basename(decodeURIComponent(parsed.pathname || ""));
...
function fetchHtmlTitleWithCurl(rawUrl) {
  return new Promise((resolve) => {
    ...
    const child = spawn3("curl", args, hiddenWindowsChildOptions2({ stdio: ["ignore", "pipe", "ignore"] }));
    const chunks = [];
    let bytes = 0;
    child.stdout.on("data", (chunk) => {
...
async function resourceBufferFromUrl(rawUrl) {
  ...
  if (/^file:/i.test(rawUrl)) {
    ...
    const buffer = await fs11.promises.readFile(resolvedPath);
...
async function saveImageFromUrl(rawUrl) {
  ...
  await fs11.promises.writeFile(result.filePath, buffer);
  return true;
}
async function writeComposerImage(buffer, ext = ".png") {
  ...
  const dir = path13.join(app.getPath("userData"), "composer-images");
  ...
  const filePath = path13.join(dir, `composer_${stamp}_${random}${safeExt}`);
...
function expandUserPath(filePath) {
  ...
  if (value.startsWith(`~${path13.sep}`) || value.startsWith("~/")) {
    return path13.join(app.getPath("home"), value.slice(2));
  }
  return value;
}
async function previewFileTarget(rawTarget, baseDir) {
  const raw = String(rawTarget || "").trim();
  const base = baseDir ? path13.resolve(expandUserPath(baseDir)) : resolveHermesCwd();
  let resolved = resolveRequestedPathForIpc(/^file:/i.test(raw) ? raw : expandUserPath(raw), {
    baseDir: base,
    purpose: "Preview target"
  });
  if (directoryExists(resolved)) {
    resolved = path13.join(resolved, "index.html");
  }
  ...
  return {
    ...
    label: path13.basename(resolved),
    language: PREVIEW_LANGUAGE_BY_EXT[ext] || "text",
    mimeType,
    path: resolved,
    previewKind,
    source: raw,
    url: pathToFileURL2(resolved).toString()
  };
}
...
async function watchPreviewFile(rawUrl) {
  ...
  const watchDir = path13.dirname(filePath);
  const targetName = path13.basename(filePath);
  const id = crypto.randomBytes(12).toString("base64url");
  let timer = null;
  const watcher = fs11.watch(watchDir, (_eventType, filename) => {
    const changedName = filename ? path13.basename(String(filename)) : "";
    ...
    timer = setTimeout(() => {
      ...
      sendPreviewFileChanged({ id, path: filePath, url: pathToFileURL2(filePath).toString() });
...
function getNativeOverlayWidth() {
  return nativeOverlayWidth({ isWindows: IS_WINDOWS2, isWsl: IS_WSL, isMac: IS_MAC });
}
...
function setAndPersistZoomLevel(window2, zoomLevel) {
  ...
  window2.webContents.send("hermes:zoom:changed", { level: next, percent: zoomLevelToPercent(next) });
...
function restorePersistedZoomLevel(window2) {
  ...
  window2.webContents.executeJavaScript(
    ...
    if (stored == null || !window2 || window2.isDestroyed()) {
      return;
    }
...
function installMediaPermissions() {
  ...
  session2.defaultSession.setPermissionCheckHandler((_webContents, permission, _origin, details) => {
...
function readDesktopConnectionConfig() {
  let mtime = null;
  try {
    mtime = fs11.statSync(DESKTOP_CONNECTION_CONFIG_PATH).mtimeMs;
  } catch {
    mtime = null;
  }
  ...
  try {
    const raw = fs11.readFileSync(DESKTOP_CONNECTION_CONFIG_PATH, "utf8");
    const parsed = JSON.parse(raw);
...
function writeDesktopConnectionConfig(config) {
  fs11.mkdirSync(path13.dirname(DESKTOP_CONNECTION_CONFIG_PATH), { recursive: true });
  writeFileAtomic(DESKTOP_CONNECTION_CONFIG_PATH, JSON.stringify(config, null, 2));
  connectionConfigCache = config;
  connectionConfigCacheMtime = fs11.statSync(DESKTOP_CONNECTION_CONFIG_PATH).mtimeMs;
}
function readActiveDesktopProfile() {
  try {
    const raw = fs11.readFileSync(DESKTOP_PROFILE_CONFIG_PATH, "utf8");
    const parsed = JSON.parse(raw);
...
function writeActiveDesktopProfile(name) {
  ...
  fs11.mkdirSync(path13.dirname(DESKTOP_PROFILE_CONFIG_PATH), { recursive: true });
...
function coerceDesktopConnectionConfig(input = {}, existing = readDesktopConnectionConfig(), options = {}) {
  ...
  const nextRemote = remoteLike ? buildRemoteBlock(remoteUrl, authMode, nextToken, cloudOrg) : { url: remoteUrl ? normalizeRemoteBaseUrl(remoteUrl) : remoteUrl, authMode, token: nextToken };
...
async function fetchJsonForProfile(profile, path14) {
  return requestJsonForProfile(profile, path14, "GET");
}
async function requestJsonForProfile(profile, path14, method, body) {
  const conn = await ensureBackend(profile);
  const url = `${conn.baseUrl}${path14}`;
...
function stopBackendChild(child) {
  ...
  try {
    if (IS_WINDOWS2 && Number.isInteger(child.pid)) {
...
function petOverlayUrl() {
  ...
  return `${pathToFileURL2(resolveRendererIndex()).toString()}?win=overlay#/`;
}
...
function createWindow() {
  ...
  if (DEV_SERVER) {
    ...
    mainWindow.loadURL(pathToFileURL2(resolveRendererIndex()).toString());
  }
...
ipcMain.handle("hermes:zoom:get", (event) => {
  ...
  const level = window2 && !window2.isDestroyed() ? window2.webContents.getZoomLevel() : 0;
...
ipcMain.handle("hermes:bootstrap:repair", async () => {
  ...
  try {
    if (fileExists(BOOTSTRAP_COMPLETE_MARKER)) {
      fs11.rmSync(BOOTSTRAP_COMPLETE_MARKER, { force: true });
    }
  } catch (error) {
...
async function interceptSessionRequestForRemote(request2) {
  ...
  if (/^\/api\/sessions\/[^/]+(\/messages)?$/.test(pathname)) {
    ...
    if (globalRemoteActive()) {
      ...
      const path14 = `${pathname}${sep}profile=${encodeURIComponent(profile)}`;
      if (method === "GET") {
        return fetchJsonForProfile(null, path14);
      }
      const body = request2.body && typeof request2.body === "object" ? { ...request2.body, profile } : { profile };
      return requestJsonForProfile(null, path14, method, body);
    }
    return void 0;
  }
  return void 0;
...
ipcMain.handle("hermes:api", async (_event, request2) => {
  ...
  const requestPath = pathWithGlobalRemoteProfile(request2.path, profile, {
...
ipcMain.handle("hermes:readFileDataUrl", async (_event, filePath) => {
  ...
  const data = await fs11.promises.readFile(resolvedPath);
...
ipcMain.handle("hermes:readFileText", async (_event, filePath) => {
  ...
  const ext = path13.extname(resolvedPath).toLowerCase();
  const handle = await fs11.promises.open(resolvedPath, "r");
...
ipcMain.handle("hermes:selectPaths", async (_event, options = {}) => {
  ...
  if (options?.defaultPath) {
    try {
      resolvedDefaultPath = path13.resolve(String(options.defaultPath));
    } catch {
...
ipcMain.handle("hermes:setting:defaultProjectDir:set", async (_event, dir) => {
  ...
  if (next) {
    try {
      fs11.mkdirSync(next, { recursive: true });
    } catch (error) {
...
ipcMain.handle("hermes:logs:reveal", async () => {
  try {
    ...
    if (!fileExists(DESKTOP_LOG_PATH)) {
      await fs11.promises.appendFile(DESKTOP_LOG_PATH, "");
    }
...
function isExecutableFile(filePath) {
  if (!filePath || !path13.isAbsolute(filePath)) {
    return false;
  }
  try {
...
function posixShellSpec(shellPath) {
  const shellName = path13.basename(shellPath);
...
function windowsPowerShellPath() {
  ...
  const builtin = path13.join(systemRoot, "System32", "WindowsPowerShell", "v1.0", "powershell.exe");
  return isExecutableFile(builtin) ? builtin : findOnPath("powershell.exe");
}
function shellSpecFor(shellPath) {
  const name = path13.basename(shellPath).toLowerCase();
...
function terminalShellCommand() {
  const override = (process.env.HERMES_DESKTOP_SHELL || (IS_WINDOWS2 ? "" : process.env.SHELL) || "").trim();
  if (override) {
...
function safeTerminalCwd(cwd) {
  const candidate = path13.resolve(String(cwd || app.getPath("home")));
  try {
    const stat = fs11.statSync(candidate);
    return stat.isDirectory() ? candidate : path13.dirname(candidate);
  } catch {
    return app.getPath("home");
  }
}
...
ipcMain.handle("hermes:fs:rename", async (_event, targetPath, newName) => {
  ...
  const dst = path13.join(path13.dirname(src), name);
  if (dst === src) {
    return { path: dst };
  }
...
ipcMain.handle("hermes:fs:writeText", async (_event, filePath, content) => {
  ...
  if (!directoryExists(path13.dirname(resolved))) {
    throw new Error("Parent directory does not exist");
  }
  await fs11.promises.writeFile(resolved, text, "utf8");
...
function resolveHermesVersion() {
  try {
    ...
    const initPath = path13.join(root, "hermes_cli", "__init__.py");
    if (fileExists(initPath)) {
      const raw = fs11.readFileSync(initPath, "utf8");
...
async function runDesktopUninstall(mode) {
  ...
  try {
    if (IS_WINDOWS2) {
      scriptPath = path13.join(app.getPath("temp"), `hermes-uninstall-${Date.now()}.cmd`);
      fs11.writeFileSync(scriptPath, buildWindowsCleanupScript(scriptArgs));
      runner = process.env.ComSpec || "cmd.exe";
      runnerArgs = ["/c", scriptPath];
    } else {
      scriptPath = path13.join(app.getPath("temp"), `hermes-uninstall-${Date.now()}.sh`);
      fs11.writeFileSync(scriptPath, buildPosixCleanupScript(scriptArgs), { mode: 493 });
      runner = "/bin/bash";
  ...
  try {
    const child = spawn3(runner, runnerArgs, {
      detached: true,
      stdio: "ignore",
      windowsHide: true
    });
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      app.setAsDefaultProtocolClient(HERMES_PROTOCOL, process.execPath, [path13.resolve(process.argv[1])]);
    } else {
