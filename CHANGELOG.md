...
**Поддержка Hermes 0.19.1 (commit f3cda0ceb18d).**

- **Совместимость:** `compat.json` и version-gate `isVersion019()` расширены с 0.19.0 на 0.19.x (0.19.0 + 0.19.1). Шаблон `/^0\.19\./` вместо жёсткого `=== '0.19.0'` — будущие патчи 0.19.x проходят автоматически.
- **ru.ts актуализирован под en.ts 0.19.1:**
  - **+2 ключа** `assistant.thread.filesChanged` / `assistant.thread.reviewChanges` (появились в 0.19.1 — UI просмотра изменений файлов в треде).
  - **−6 ключей** `zones.split/move/dirUp/dirDown/dirLeft/dirRight` (были добавлены в v1.2.1 «с запаском», но в релиз 0.19.1 не вошли — контракт ru ⊆ en нарушался).
- **Контракт:** 2599/2599 ключей ru.ts ↔ en.ts 0.19.1 — 0 расхождений. npm test: 19/19 зелёных.

## v1.2.1 (2026-07-30)

**Релиз-фиксы: npm repository.url + pkg fix.**

- Исправлен `repository.url` в package.json для корректной публикации в npm.
- README refresh + CI skip-if-published guard.

## v1.2.0 (2026-07-30)

**Консолидация переводов: мёрдж двух источников с указанием соавторства.**

- **База перевода заменена** на каталог DrMaks22 (PR #72250 NousResearch/hermes-agent) — native-reviewed, 2755/2755 leaf-parity с текущим `en.ts`.
- **Наш вклад сохранён** поверх базы: 84 ключа которых не было у DrMaks22 + литературные улучшения (правильное использование буквы «ё» в кратких причастиях м.р.: подключён, отключён, завершён; исправление «подключен» → «подключён»).
- **Атрибуция:** DrMaks22 указан как соавтор и основной референс перевода. Заголовок `ru.ts` содержит комментарий с описанием источников.
- **Покрытие:** 2484 ключа → 100% покрытия upstream `en.ts` (было 96.4%).
- **Структура:** переведён на формат `defineLocale` + `ruOverrides` (соответствует upstream-стилю `ja.ts`/`zh.ts`).
- Лингвистические улучшения: «Трансляция» вместо кальки «Стриминг», «Дерево субагентов» вместо «Дерево порождений», страдательный залог в action-titles («Файл изменён» вместо «Изменил файл»), «Создать ответвление» вместо «Ветвь в новом чате».


## v1.1.5 (2026-07-30)

**UX-фикс: уточнены инструкции о пересборке (главная причина «русский не применился»).**

- **Корневая проблема:** после `hermes-ru install` при запущенном Hermes сообщение говорило «просто запустите `hermes desktop` (само соберёт при запуске)». Пользователь воспринимал это как «перезапустите приложение» — но обычный перезапуск `.exe` НЕ вызывает пересборку. Content-hash стемп сравнивается только при запуске через CLI-команду `hermes desktop` в терминале.
- **Исправлено:** все сообщения (`install`, `status`, `help`) и README теперь чётко говорят:
  1. Закрыть Hermes полностью (крестиком)
  2. Открыть терминал (cmd/PowerShell)
  3. Выполнить `hermes desktop --force-build` (2–10 мин)
- Добавлен явный callout в README: «обычный перезапуск `.exe` НЕ вызывает пересборку».
- Раздел «Решение проблем» переписан: конкретные шаги вместо туманного «соберёт сам».
...
**Критичные фиксы components-patch, найденные прогоном против РЕАЛЬНОГО дерева 0.19.0 (суб-агентные якоря оказались фантазийными — фикстуры были написаны под патч, а не под реальный код):**

- **Якоря JSX text-node:** реальные компоненты хранят текст многострочно (`>\n<indent>Text\n`), а не инлайн (`>Text<`). Все 25 сайтов переведены на `replaceJsxText()` (обе формы). Отдельные формы: `Default: <span…>`, `'Undo'` внутри тернарника `{busy ? 'Undoing…' : 'Undo'}`.
- **RU-переводы новых ключей:** v1.1.0/v1.1.1 добавляли ключи только в en.ts — русская локаль падала бы в английский fallback для всех 158 новых строк. Теперь `patchRuTs()` вставляет зеркальные RU-блоки (`src/components-patch-ru-blocks.js`, генерируется из PR-worktree `scripts/gen-ru-blocks.js`: 140 ключей сопоставлено с переводом PR + 18 ручных).
- **Проверка в песочнице:** `scripts/sandbox-verify.js` — прогон патча против КОПИИ реального дерева до касания установки; en 2676 = ru 2678, все t()-ссылки покрыты ключами.
- **Фикстуры тестов приведены к реальным формам** (Enabled/Default:/Undo — байт-в-байт как в 0.19.0).
