...
* install запускает официальную сборку (hermes desktop --build-only), только
  если Hermes закрыт. Если Hermes запущен — закройте приложение полностью
  (крестиком), откройте терминал (cmd/PowerShell) и выполните:

    hermes desktop --force-build

  Эта команда пересоберёт приложение (2–10 мин) и запустит его.
  Важно: обычный перезапуск .exe НЕ вызывает пересборку — нужна команда в терминале.
`;
