...
|---|---|---|
| Установка | Скачать архив → распаковать → `install.ps1` → `npm run build` (нужен Node dev-окружение, 10+ минут) | `npm i -g @anatolijlaptev1991/hermes-ru` → `hermes-ru install` |
...
| Требования | Node.js, npm, git, исходники Hermes (для пересборки) | Только Node.js 18+ |
