...
- Если Hermes запущен — патч и язык применятся сразу, а пересборка произойдёт автоматически при следующем `hermes desktop` / `hermes update`. Либо после закрытия выполните `hermes-ru build`.
...
**`hermes-ru status` говорит «ждёт пересборки».** Закройте Hermes и: `hermes-ru build` (или просто запустите `hermes desktop` — соберёт сам при запуске).
