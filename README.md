...
Локаль `ru` движется в основной репозиторий Hermes через два PR:
- **[#74691](https://github.com/NousResearch/hermes-agent/pull/74691)** — i18n-wiring захардкоженных поверхностей (MoA, Billing, Custom Endpoints). Locale-neutral, нет конкурентов.
- **[#72250](https://github.com/NousResearch/hermes-agent/pull/72250)** — консолидированный русский каталог (DrMaks22). Наш PR [#71573](https://github.com/NousResearch/hermes-agent/pull/71573) сузился до генератора/контракт-теста.

Когда PR примут — пакет станет ненужным, и это будет победа.
...
| Hermes | hermes-ru |
|--------|-----------|
| 0.19.0 | **1.2.0** (структурные якоря + мёрдж переводов) |
| 0.19.0 | 1.0.0–1.1.5 (структурные якоря) |
...
├── src/i18n/ru.ts          # перевод (2755 ключей, мёрдж DrMaks22 + наш)
...
4. **v1.1.0–v1.1.4:** i18n-wiring захардкоженных поверхностей (MoA, Billing, Custom Endpoints) — 257 новых ключей, перевод которых до этого рендерил английский для всех локалей. Компоненты-патчи с идемпотентностью и rollback.
5. **v1.1.5:** UX-фикс — уточнены инструкции о пересборке. Обычный перезапуск `.exe` НЕ вызывает пересборку; нужен `hermes desktop --force-build` в терминале.
6. **v1.2.0:** консолидация переводов. Мёрдж двух независимых каталогов: DrMaks22 (native-reviewed, PR #72250) как база + наш вклад (84 доп. ключа, ё-fidelity). 2755 ключей, 100% покрытие.
