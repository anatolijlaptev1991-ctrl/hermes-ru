...
1. `hermes-ru install` копирует `ru.ts` в `src/i18n/`, патчит `types.ts`, `catalog.ts`, `languages.ts` (регистрирует локаль `ru` в нативной системе i18n Hermes) и запускает `npm run build`.
...
6. **v0.20.4** — преflight-проверка `electron.exe` (213 МБ бинарник не скачивается при `npm install` → белый экран). Launcher автоматически скачивает через `electron/install.js`.
